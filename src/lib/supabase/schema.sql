...
alter table public.products add constraint products_availability_status_check check (
  availability_status in ('Active', 'Inactive', 'Sold Out')
);
...
set
  review_status = case
    ...
    when review_status in ('On Hold', 'on_hold') or status in ('On Hold', 'on_hold') then 'On Hold'
...
drop policy if exists "product_media_storage_public_read" on storage.objects;
drop policy if exists "product_media_storage_auth_insert_own_folder" on storage.objects;
drop policy if exists "product_media_storage_auth_update_own_folder" on storage.objects;
drop policy if exists "product_media_storage_auth_delete_own_folder" on storage.objects;

create policy "product_media_storage_public_read"
on storage.objects for select to anon, authenticated
using (bucket_id = 'product-media');

create policy "product_media_storage_auth_insert_own_folder"
...
create policy "product_media_storage_auth_update_own_folder"
...
create policy "product_media_storage_auth_delete_own_folder"
